...
tts_repo_dir = os.path.join(script_dir, "DocPOI_repo", "XTTS-v2")
...
def setup_ssh_key():
    ...
    if os.name == 'nt':  # Windows
        ...
        key_path = os.path.join(script_dir, 'id_ed25519')
    else:  # Linux/MacOS
    ...
    # Move the SSH private key to the .ssh directory
    destination_key_path = os.path.join(ssh_dir, 'id_ed25519')
    if not os.path.exists(destination_key_path):
        print(f"Moving SSH private key to {destination_key_path}...")
        shutil.move(key_path, destination_key_path)
    else:
        print(f"SSH key already exists at {destination_key_path}.")

    # Set the correct permissions for the private key
    if os.name != 'nt':  # Only necessary on Linux/MacOS
        run_cmd(f"chmod 600 {destination_key_path}")

    # Add the SSH key to the SSH agent
    if os.name == 'nt':  # Windows
        run_cmd("eval $(ssh-agent -s)", capture_output=True)
        run_cmd(f'ssh-add {destination_key_path}')
    else:  # Linux/MacOS
        run_cmd("eval $(ssh-agent -s)", capture_output=True)
        run_cmd(f"ssh-add {destination_key_path}")

    # Verify SSH connection
    print("Verifying SSH connection to GitHub...")
    run_cmd("ssh -T git@github.com")
...
def install_dependencies():
    ...
    if not os.path.exists(repo_dir):
        print("Cloning DocPOI_repo...")
        run_cmd("git clone git@github.com:Darthph0enix7/DocPOI_repo.git")
    else:
        print("DocPOI_repo already exists.")
...
if __name__ == "__main__":
    ...
    if args.update:
        ...
        if not os.path.exists("DocPOI_repo/"):
